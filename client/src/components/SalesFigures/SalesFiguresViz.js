...
const SalesFiguresViz = ({salesFigures, province, type}) => {
    ...
    const xScale = scaleLinear().domain(extent(salesFigures.filter(figure => figure["GEO"] === `${province}` && figure["Vehicle type"] === `${type}`), xValue)).range([0, innerWidth]).nice();
    const yScale = scaleLinear().domain(extent(salesFigures.filter(figure => figure["GEO"] === `${province}` && figure["Vehicle type"] === `${type}`), yValue)).range([innerHeight, 0]).nice();

    const fuelTypes = ["All fuel types", "Gasoline", "Diesel", "Battery electric", "Hybrid electric", "Plug-in hybrid", "Other fuel types"]
    const colors = {
        "All fuel types": "blue",
        "Gasoline": "red",
        "Diesel": "orange",
    ...
    return (
        <svg ref={salesFiguresRef}>
            <g transform={`translate(${margin.left}, 20)`}>
                <SalesFiguresXTicks xScale={xScale} innerHeight={innerHeight}></SalesFiguresXTicks>
                <SalesFiguresYLabel innerHeight={innerHeight}></SalesFiguresYLabel>
                <SalesFiguresYTicks yScale={yScale} innerWidth={innerWidth}></SalesFiguresYTicks>
                <SalesFiguresXLabel innerWidth={innerWidth} innerHeight={innerHeight}></SalesFiguresXLabel>
                <SalesFiguresLines fuelTypes={fuelTypes} colors={colors} province={province} type={type} salesFigures={salesFigures} xValue={xValue} yValue={yValue} xScale={xScale} yScale={yScale} />
                <SalesFiguresPoints fuelTypes={fuelTypes} colors={colors} province={province} type={type} salesFigures={salesFigures} xValue={xValue} yValue={yValue} xScale={xScale} yScale={yScale} />
